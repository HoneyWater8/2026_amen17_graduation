...
3. **Root Directory는 반드시 `FE`.** CLI 배포(`cd FE && vercel`)는 그 폴더를 통째로 업로드하므로 `.`이어도 동작하지만, **Git 연동 빌드는 레포 루트를 기준**으로 하므로 `.`이면 `package.json`을 못 찾아 실패합니다.
   `vercel project`에 설정 변경 커맨드가 없어 **CLI로는 못 바꿉니다.** 대시보드 Settings → Build & Deployment에서 지정해야 합니다. (2026-09-07 `FE`로 설정 완료)
